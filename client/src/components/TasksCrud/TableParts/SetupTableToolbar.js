...
export default function SetupTableToolbar({
  tasksSelected,
  setTasksFilter,
  tasksFilter,
}) {
  const projectSprintCount = useSelector(
    ...
    if (user) {
      ...
      if (e.target.name === "sprintId") {
        if (parseInt(e.target.value) === 0) {
          ...
          const sprint =
            parseInt(e.target.value) > projectSprintCount
              ? projectSprintCount
        ...
        e.target.value === "All Priorizations"
      ) {
        filterCb = (task) => {
          return task;
        };
      ...
      {tasksSelected.length > 0 ? (
        <>
          <div className={styles.toolBarFields}>
            <select
              value={
                dynamicFields.priorization !== null
                  ? dynamicFields.priorization
                  : "Don't Change"
              }
              name="priorization"
            ...
            <label style={{ alignSelf: "center" }}>Sprint</label>
            <input
              type="number"
              name="sprintId"
              value={
                dynamicFields.sprintId !== null ? dynamicFields.sprintId : 0
              }
              onChange={(e) => handleTaskFieldsChange(e)}
              min="0"
          ...
          <div className={styles.toolBarOptions}>
            <Tooltip title="Confirm" className={styles.toolBarBtn}>
              <IconButton onClick={() => handleSubmitSelectedTasks()}>
                <BsCheckBox />
              </IconButton>
            </Tooltip>
        ...
        <div className={styles.toolBarFilter}>
          <Tooltip
            title="Filter list"
            className={
              showFilterList ? styles.toolBarBtnActive : styles.toolBarBtn
            }
          >
          ...
          <div
            className={
              showFilterList ? styles.showFilterList : styles.hideFilterList
            }
          >
            <select
              name="priorization"
            ...
            <div className={styles.toolBarSelectUsers}>
              ...
              >
                ...
                {filterListFields.user !== null ? (
                  <p>{filterListFields.user.name}</p>
                ) : (
                  <p>All Users</p>
                )}
              </div>
